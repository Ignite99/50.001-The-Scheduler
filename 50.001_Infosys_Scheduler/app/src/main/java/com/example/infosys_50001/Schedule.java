...
public class Schedule{
    ...
    public boolean contains(TimeBlock period){
        for (TimeBlock tb:this.data){
            if (tb.toObj().isAfter(period.fromObj())){
                if (tb.contains(period)){
                    return true;
                }
            }
        }
...
//     }
    ...
    public java.util.ArrayList<String> getRecycleSchedArrayList(java.time.LocalDateTime start_time,java.time.LocalDateTime end_time,int minutes_period){
        TimeBlock init = new TimeBlock(start_time,end_time);
        java.util.ArrayList<String> result = new java.util.ArrayList<>();
        int period_count = ((int) init.duration()/minutes_period)-1;
        for (int i=0;i<=period_count;i++){
            TimeBlock to_test = init.pos(i,minutes_period);
            int contains = 0;
            if (this.contains(to_test)){
                contains = 1;
            }
            result.add(to_test.fromObj().toString());
            result.add(to_test.toObj().toString());
            result.add(Integer.toString(contains));
        }
        return result;
    }
