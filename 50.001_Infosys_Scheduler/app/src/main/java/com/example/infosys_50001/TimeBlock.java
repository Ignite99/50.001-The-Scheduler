...
public class TimeBlock{
    ...
    public Boolean contains(TimeBlock target){
        //returns true if TimeBlockA.in(TimeBlockB)
        return (
                (this.from_obj.isBefore(target.from_obj) | this.from_obj.isEqual(target.from_obj)) &&
                        (this.to_obj.isAfter(target.to_obj) | this.to_obj.isEqual(target.to_obj))
        );
    }
    public TimeBlock pos(int pos, int minutes){
        java.time.LocalDateTime pos_start = from_obj.plusMinutes((long) pos *minutes);
        java.time.LocalDateTime pos_end = pos_start.plusMinutes(minutes-1);
        pos_end = pos_end.plusSeconds(59);
        return new TimeBlock(pos_start,pos_end);
    }
